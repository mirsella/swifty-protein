  ...
  switch (style) {
    case "ball-stick":
      ...
      if (selectedAtom.value) {
        viewer.setStyle(
          { serial: selectedAtom.value.serial },
          {
            stick: {},
...
const takeAndShareScreenshot = async () => {
  try {
    ...
    const canvas: HTMLCanvasElement | null =
      viewerContainer.value?.querySelector("canvas");
...
onMounted(async () => {
  try {
    ...
    switchStyle("ball-stick"); // apply default style
...
<template>
  <div class="flex-grow flex flex-col items-center justify-center mx-4 my-4">
    ...
    <div
      class="flex-grow relative w-full max-w-4xl h-80 overflow-hidden rounded-box"
    >
      ...
      <template v-if="!loading">
        ...
        <div class="absolute bottom-2 left-2 z-10 flex gap-2">
          <button
            ...
            class="btn btn-sm"
            :class="{ 'btn-primary': currentStyle === style }"
          >
            {{ style }}
          </button>
        </div>
        ...
        <div
          v-if="selectedAtom"
          class="absolute top-0 right-0 z-10 m-1 p-1 border rounded shadow-lg bg-base-100"
        >
